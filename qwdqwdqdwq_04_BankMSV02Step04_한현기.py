...
#-----------------------------------------------------------------------------------------------------------------------------------------------			
	...
	def customIns(self):
		...
		vId = int(self.AccDtoList[len(self.AccDtoList)-1].getId()[-3:])+1
		vNum = int(self.AccDtoList[len(self.AccDtoList)-1].getNumber()[-3:])+1
		vZero = ""
		for zeroChk in range(3-len(str(vId))):
			vZero+='0'
		vId = self.AccDtoList[len(self.AccDtoList)-1].getId()[:4] +vZero +str(vId)
		vNum =self.AccDtoList[len(self.AccDtoList)-1].getNumber()[:8]+vZero +str(vNum)
		print(vId,vNum)
